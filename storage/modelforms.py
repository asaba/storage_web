...
class SelectBEForm(Form):
    ...
    codBE = forms.CharField(widget=forms.Select(choices=[(be["backend"], be["backend"]) for be in
                                                         Tdays.objects.all().order_by().values("backend").annotate(
                                                             n=Count("pk"))]), label="Back End")
