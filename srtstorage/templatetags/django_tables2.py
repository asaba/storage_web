...
class RenderTableNode(Node):
    ...
    def __init__(self, table, tmp_template=None):
    ...
    def render(self, context):
        ...
        if self.template:
            tmp_template = self.template.resolve(context)
        else:
            tmp_template = table.template

        if isinstance(tmp_template, six.string_types):
            tmp_template = get_template(tmp_template)
        else:
            # assume some iterable was given
            tmp_template = select_template(tmp_template)
